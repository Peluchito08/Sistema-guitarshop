...
export async function GET(req: Request) {
	...
	try {
		const items = await prisma.producto_compra.findMany({
			select: {
			...
			orderBy: { id_producto_compra: "desc" },
		});

		return jsonCors(items, { status: 200 });
	} catch (error) {
		console.error("Error GET /producto_compra:", error);
		return jsonCors(
...
export async function POST(req: Request) {
	...
	return jsonCors(
		{
			error:
				"Operación no soportada: para registrar compras use /api/compra para mantener stock/kardex/totales.",
		},
		{ status: 409 }
	);
}
