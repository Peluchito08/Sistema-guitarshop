...
export async function obtenerDashboard() {
  ...
  const productLookup = new Map(
    productMap.map((producto) => [producto.id_producto, producto])
  );
  ...
  const totalCostosMes = detallesDelMes.reduce((acc, detalle) => {
    const costo = detalle.producto?.precio_compra
      ? toNumber(detalle.producto.precio_compra)
      : 0;
  ...
  const detalleCuotas = cuotasVencidasDetalle.map((cuota) => {
    ...
    const pendiente =
      toNumber(cuota.monto_cuota) - toNumber(cuota.monto_pagado);
